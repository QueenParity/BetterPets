...
public class WaterFilterRenderer implements BlockEntityRenderer<WaterFilterBlockEntity>
{
    ...
    @Override
    public void render(WaterFilterBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        poseStack.pushPose();
        poseStack.translate(0.5, 0.5, 0.5);
        Direction direction = blockEntity.getBlockState().getValue(WaterFilterBlock.DIRECTION);
        poseStack.mulPose(Vector3f.YP.rotationDegrees(direction.get2DDataValue() * -90F - 90F));
        poseStack.translate(-0.5, -0.5, -0.5);
        float heightWater = (float) (14.0 * (blockEntity.getWaterLevel() / (double) blockEntity.getWaterTank().getCapacity()));
        float heightFilteredWater = (float) (14.0 * (blockEntity.getFilteredWaterLevel() / (double) blockEntity.getFilteredWaterTank().getCapacity()));
        if(heightWater > 0)
        {
            this.drawFluid(blockEntity.getWaterFluidStack().getFluid(), blockEntity, poseStack, buffer, 1.01F * 0.0625F, 1.01F * 0.0625F, 1.01F * 0.0625F, (14 - 0.02F) * 0.0625F, heightWater * 0.0625F, (6 - 0.02F) * 0.0625F);
        }
        if(heightFilteredWater > 0)
        {
            this.drawFluid(blockEntity.getFilteredWaterFluidStack().getFluid(), blockEntity, poseStack, buffer, 1.01F * 0.0625F, 1.01F * 0.0625F, 9.01F * 0.0625F, (14 - 0.02F) * 0.0625F, heightFilteredWater * 0.0625F, (6 - 0.02F) * 0.0625F);
        }
        poseStack.popPose();
    }
    
    private void drawFluid(Fluid fluid, WaterFilterBlockEntity be, PoseStack poseStack, MultiBufferSource bufferSource, float x, float y, float z, float width, float height, float depth)
    {
