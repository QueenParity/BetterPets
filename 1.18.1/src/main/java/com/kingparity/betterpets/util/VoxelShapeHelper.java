...
public class VoxelShapeHelper
{
    ...
    public static VoxelShape setMaxHeight(VoxelShape source, double height)
    {
        ...
        {
            VoxelShape shape = Shapes.create(minX, minY, minZ, maxX, height, maxZ);
            result.set(Shapes.join(result.get(), shape, BooleanOp.OR));
        });
    ...
    public static VoxelShape limitHorizontal(VoxelShape source)
    {
        ...
        source.forAllBoxes((minX, minY, minZ, maxX, maxY, maxZ) ->
        {
            VoxelShape shape = Shapes.create(limit(minX), minY, limit(minZ), limit(maxX), maxY, limit(maxZ));
            result.set(Shapes.join(result.get(), shape, BooleanOp.OR));
        });
        return result.get().optimize();
    }
