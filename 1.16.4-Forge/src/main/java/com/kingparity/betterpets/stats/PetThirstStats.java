...
public class PetThirstStats
{
    ...
    public void tick(BetterWolfEntity betterWolf)
    {
        ...
        exhaustMultiplier *= betterWolf.world.getBiome(betterWolf.getPosition()).getCategory() == Biome.Category.DESERT ? 2.0f : 1.0f;
        
        if(betterWolf.areEyesInFluid(FluidTags.WATER) || betterWolf.isInWater())
        {
            addExhaustion(0.03f * ms * 0.003f * exhaustMultiplier);
        }
        else if(betterWolf.isOnGround())
        {
        ...
        {
            if(poisonTimer++ < /*Reference.POISON_DURATION*/(60 * 20))
            {
    ...
    {
        betterWolf.world.getPlayers().forEach((player) -> PacketHandler.instance.sendTo(new MessageUpdateThirstStats(betterWolf.getEntityId(), this.thirstLevel, this.thirstSaturationLevel, this.thirstExhaustionLevel, this.poisoned), ((ServerPlayerEntity)player).connection.netManager, NetworkDirection.PLAY_TO_CLIENT));
    ...
    {
        double x = betterWolf.getPosX() - betterWolf.prevPosX;
        double y = betterWolf.getPosY() - betterWolf.prevPosY;
        double z = betterWolf.getPosZ() - betterWolf.prevPosZ;
